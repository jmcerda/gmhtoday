...
class SlickBrowser implements SlickBrowserInterface {
   ...
   */
  public function __construct(SlickAdminInterface $slick_admin, SlickFormatterInterface $formatter) {
  ...
  public function widgetEntiyBrowserFormAlter(&$element, FormStateInterface $form_state, $context) {
    ...
    $third_party_settings = $context['widget']->getThirdPartySettings('slick_browser');
    ...
    switch ($plugin_id) {
      case 'entity_browser_entity_reference':
        $this->widgetEntityBrowserEntityReferenceFormAlter($element, $form_state, $context);
        break;

      case 'entity_browser_file':
        $this->widgetEntityBrowserFileFormAlter($element, $form_state, $context);
        break;
    ...
    if ($plugin_id == 'entity_browser_file') {
      ...
      $attachments = isset($element['current']['#attached']) ? NestedArray::mergeDeep($element['current']['#attached'], $load) : $load;
    }
   ...
   * Implements hook_field_widget_WIDGET_TYPE_form_alter().
   */
  public function widgetEntityBrowserEntityReferenceFormAlter(&$element, FormStateInterface $form_state, $context) {
    $widget_settings = $context['widget']->getSettings();
   ...
   * Implements hook_field_widget_WIDGET_TYPE_form_alter().
   */
  public function widgetEntityBrowserFileFormAlter(&$element, FormStateInterface $form_state, $context) {
    $widget_settings = $context['widget']->getSettings();
    ...
    $settings = self::buildThirdPartySettings($context['widget']);
    if (empty($settings['style'])) {
      return;
    }
    ...
    foreach ($entities as $delta => $entity) {
      ...
      $display = $this->buildPreview($data, $entity, $entity->getFilename());
  ...
  public function buildSettingsForm(array &$element, $definition) {
    ...
    if (isset($element['style'])) {
      ...
      // @todo: Single image preview to not use table.
    ...
    // Toggle visibility.
    // @todo: Refine this insanity.
    $states['visible']['select[name$="[style]"]'] = ['value' => 'slick'];
    $single['visible']['select[name$="[style]"]'] = ['!value' => 'single'];
    $grids = ['grid', 'grid_header', 'grid_medium', 'grid_small'];
    $both = [
      'grid',
      'grid_header',
      'grid_medium',
    ...
    foreach ($settings as $item => $value) {
      ...
      if (in_array($item, $grids)) {
        $element[$item]['#states'] = $single;
      }

      if (!isset($element[$item]['#type']) || in_array($item, $both)) {
        continue;
      }

      if (in_array($item, $thumbs)) {
        $element[$item]['#states'] = $states;
      }

      if (isset($element[$item]['#type']) && in_array($element[$item]['#type'], $excludes)) {
        continue;
      }

      if (isset($element[$item])) {
        if (isset($element[$item]['#states'])) {
          $element[$item]['#states'] = NestedArray::mergeDeepArray([$element[$item]['#states'], $states], TRUE);
        }
        else {
          $element[$item]['#states'] = $states;
        }
    ...
    if (isset($element['visible_items'])) {
      $element['visible_items']['#states']['visible'] = [
        'select[name$="[grid]"]' => ['!value' => ''],
        'select[name$="[style]"]' => ['value' => 'slick'],
      ];
    }
  }
